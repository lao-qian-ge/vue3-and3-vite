...
import RouteView from "@/layouts/routerView/routerView.vue";
  ...
  return routerMap.map((item) => {
    const currentRouter: RouteRecordRaw = {
      ...
      component:
        ...
        constantRouterComponents[(item.component || item.key) as ("BasicLayout" | "RouteView")] ||
        // eslint-disable-next-line prettier/prettier
    ...
    // 重定向
    item.redirect && ((currentRouter as RouteRecordRaw).redirect = item.redirect);
    // 是否有子菜单，并递归处理
    if (item.children && item.children.length > 0) {
      (currentRouter as RouteRecordRaw).children = generator(
        item.children,
        currentRouter
      );
    }
